 ...
 *******************************************************************************/
package cmcc.gz.lib.controls.pulltorefresh;
...
import cmcc.gz.lib.controls.pulltorefresh.internal.FlipLoadingLayout;
import cmcc.gz.lib.controls.pulltorefresh.internal.LoadingLayout;
import cmcc.gz.lib.controls.pulltorefresh.internal.RotateLoadingLayout;
import cmcc.gz.lib.controls.pulltorefresh.internal.Utils;
import cmcc.gz.lib.controls.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			Log.i("pulltorefresh信息", "mState=" + mState);
			if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
				if ((mState != State.RELEASE_TO_REFRESH)) {
					//20130730 杨健修改之二 为了避免下面的修改之一中导致的在下拉刷新和松开立即刷新的提示中重复变化
					//特别加上上面的判断
					setState(State.PULL_TO_REFRESH);
				}
				//} else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {
			} else if (mState == State.PULL_TO_REFRESH || itemDimension < Math.abs(newScrollValue)) {
				//20130730 杨健修改之一， 原来上面这行是并且&&关系，为了实现下拉就刷新，所以改成了或者||
